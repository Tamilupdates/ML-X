...
<i>Send /stop to Stop Process</i>""")
        ...
        async with session_lock:
            if session_dict['CONFIRM_PHN'].lower() in ['y', 'yes']:
                break
    try:
        pyro_client = Client(f"ML-X-{message.from_user.id}", api_id=api_id, api_hash=api_hash)
...
<i>Send /stop to Stop Process</i>""")
 ...
 <i>Send /stop to Stop Process</i>""")
    ...
    try:
        await aioremove(f'ML-X-{message.from_user.id}.session')
        await aioremove(f'ML-X-{message.from_user.id}.session-journal')
    except Exception:
        pass
